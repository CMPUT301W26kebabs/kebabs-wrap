...
public class AdminEventDetailDialog extends BottomSheetDialogFragment {
    ...
    public static AdminEventDetailDialog newInstance(String eventId) {
        AdminEventDetailDialog dialog = new AdminEventDetailDialog();
    ...
    public void setOnEventRemovedListener(Runnable listener) { this.onEventRemovedListener = listener; }
    ...
    private void loadEventData() {
        repository.fetchEventById(eventId, new FirebaseRepository.OnDocumentLoadedListener() {
            ...
            @Override public void onError(Exception e) { if (isAdded()) { Toast.makeText(getContext(), "Failed to load event", Toast.LENGTH_SHORT).show(); dismiss(); } }
        });
    }
    ...
    private void populateUI(DocumentSnapshot doc) {
        ...
        Long wlCap = doc.getLong("maxWaitlistCapacity");
        tvWaitlistCap.setText("Waitlist Limit: " + (wlCap != null ? wlCap : "Unlimited"));
    ...
    private void confirmRemove() {
        new AlertDialog.Builder(requireContext()).setTitle("Remove Event").setMessage("This will hide the event from all users. Continue?")
                .setPositiveButton("Remove", (d, w) -> {
                    ...
                    repository.softDeleteEvent(eventId, adminId, new FirebaseRepository.OnOperationCompleteListener() {
                        @Override public void onSuccess() { if (isAdded()) { Toast.makeText(getContext(), "Event removed", Toast.LENGTH_SHORT).show(); if (onEventRemovedListener != null) onEventRemovedListener.run(); dismiss(); } }
    ...
    private void restoreEvent() {
        repository.restoreEvent(eventId, new FirebaseRepository.OnOperationCompleteListener() {
            @Override public void onSuccess() { if (isAdded()) { Toast.makeText(getContext(), "Event restored", Toast.LENGTH_SHORT).show(); if (onEventRemovedListener != null) onEventRemovedListener.run(); dismiss(); } }
            @Override public void onError(Exception e) { if (isAdded()) Toast.makeText(getContext(), "Error: " + e.getMessage(), Toast.LENGTH_LONG).show(); }
        });
    }
}
