...
public class AdminImagesActivity extends AppCompatActivity implements ImageAdapter.OnImageActionListener {
        ...
        repository.fetchAllEvents(new FirebaseRepository.OnDocumentsLoadedListener() {
            @Override public void onLoaded(List<DocumentSnapshot> documents) {
                ...
                for (DocumentSnapshot doc : documents) {
                    ...
                    if (posterUrl != null && !posterUrl.isEmpty() && (isDeleted == null || !isDeleted))
                        withPosters.add(doc);
                }
    ...
    @Override public void onImageRemoveClick(DocumentSnapshot eventDoc) {
        new AlertDialog.Builder(this).setTitle("Remove Image")
            .setMessage("Remove the poster for \"" + eventDoc.getString("name") + "\"?")
            .setPositiveButton("Remove", (dialog, which) -> {
                repository.updateEventPosterUrl(eventDoc.getId(), null,
                    aVoid -> { Toast.makeText(this, "Image removed", Toast.LENGTH_SHORT).show(); loadImages(); },
