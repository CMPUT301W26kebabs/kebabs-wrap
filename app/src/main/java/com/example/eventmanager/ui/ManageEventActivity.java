...
import com.example.eventmanager.adapter.EntrantAdapter;
...
public class ManageEventActivity extends AppCompatActivity {
    ...
    private void loadAttendees() {
        if (previewWaitingMode && "waiting".equals(currentTab)) {
            List<Entrant> preview = getPreviewEntrants();
        ...
        db.collection("events").document(eventId).collection(collection).get()
                .addOnSuccessListener(qs -> {
                    List<Entrant> attendeeRows = new ArrayList<>();
                    for (DocumentSnapshot doc : qs.getDocuments()) {
                        ...
                        Entrant e = new Entrant(deviceId,
                    ...
                    List<Entrant> groupedEntrants = buildGroupedEntrants(attendeeRows);
    ...
    private List<Entrant> buildGroupedEntrants(List<Entrant> attendees) {
        List<Entrant> grouped = new ArrayList<>();
        ...
        for (Entrant attendee : attendees) {
            ...
            if (!letter.equals(currentHeader)) {
                ...
                Entrant header = new Entrant("", letter, "", "header");
    ...
    private void shareCurrentTabList() {
        if (previewWaitingMode && "waiting".equals(currentTab)) {
            StringBuilder sb = new StringBuilder();
            for (Entrant e : getPreviewEntrants()) {
    ...
    private List<Entrant> getPreviewEntrants() {
